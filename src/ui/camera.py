...
REAL_CAR_WIDTH = (
    1.8  # meters; used for distance est. for class 'car'. Adjust if needed.
)
...
def camera_grabber():
    # If configured, use a video file as the source (loop on end-of-file)
    if USE_VIDEO_FILE:
        if not VIDEO_FILE.exists():
            # If the path is invalid, fallback to camera (existing behavior)
            print(
                f"camera_grabber: WARNING - video file {VIDEO_FILE} not found, falling back to camera device"
            )
        else:
            cap = cv2.VideoCapture(str(VIDEO_FILE))
            # prefer the configured width/height if the file has a different resolution
            cap.set(cv2.CAP_PROP_FRAME_WIDTH, CAM_WIDTH)
            cap.set(cv2.CAP_PROP_FRAME_HEIGHT, CAM_HEIGHT)
            while True:
                ret, frame = cap.read()
                if not ret:
                    # loop to beginning on EOF
                    cap.set(cv2.CAP_PROP_POS_FRAMES, 0)
                    continue
                frame_rgb = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)
                if not frame_q.full():
                    frame_q.put(frame_rgb)
            # unreachable fallback; keep structure explicit
            return
...
def detection_worker():
    ...
    while True:
        ...
        results = model(
            frame, imgsz=DETECT_IMGSZ, conf=0.35, classes=None, device="cpu"
        )
        det_list = []
...
class Viewer(mglw.WindowConfig):
    ...
    def __init__(self, **kwargs):
        ...
        fs = (Path(__file__).parent / "shaders" / "simple_flag.glsl").read_text()
